...
func createWorkoutPlan(workoutPlan models.WorkoutPlan, res http.ResponseWriter, req *http.Request) error {
	...
	// checking to make sure workout plan doesn't already exist
	workoutPlanErr := coachCollection.FindOne(context.Background(), bson.D{primitive.E{Key: "workoutPlans.name", Value: workoutPlan.Name}})

	if workoutPlanErr.Err() != mongo.ErrNoDocuments {
		errString := "workout plan with that name already exists"
		http.Error(res, errString, 400)
		return errors.New(errString)
	}

	fmt.Println(coach.WorkoutPlans, workoutPlan)
	newWorkoutPlans := append(coach.WorkoutPlans, workoutPlan)
	...
	_, coachUpdateErr := coachCollection.UpdateByID(context.Background(), coachID, workoutPlanUpdate)
